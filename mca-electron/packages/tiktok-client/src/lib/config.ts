import { CustomSignerConfig, WebcastConfig } from '../types/client';


import { generateDeviceId, userAgentToDevicePreset } from './utilities';
...
const parseCustomSignHeaders = (): Record<string, string> => {
    const rawHeaders = process.env.CUSTOM_SIGN_HEADERS;
    if (!rawHeaders) {
        return {};
    }
    try {
...
export const CustomSignConfig: Partial<ClientConfiguration> = {
    basePath: process.env.CUSTOM_SIGN_API_URL || '',
    apiKey: process.env.CUSTOM_SIGN_API_KEY,
    baseOptions: {
        headers: {
            'User-Agent': `tiktok-live-connector/${VERSION} ${process.platform}`,
            ...parseCustomSignHeaders()
        },
        validateStatus: () => true
    }
};

export const buildCustomSignConfig = (override?: CustomSignerConfig): Partial<ClientConfiguration> => {
    const mergedHeaders = {
        ...(CustomSignConfig.baseOptions?.headers || {}),
        ...(override?.headers || {})
    };

    return {
        ...CustomSignConfig,
        ...(override?.basePath !== undefined ? { basePath: override.basePath } : {}),
        ...(override?.apiKey !== undefined ? { apiKey: override.apiKey } : {}),
        baseOptions: {
            ...(CustomSignConfig.baseOptions || {}),
