...
export type FetchSignedWebSocketFromCustomRouteParams = FetchSignedWebSocketParams;

export class FetchSignedWebSocketFromCustomRoute extends Route<FetchSignedWebSocketFromCustomRouteParams, ProtoMessageFetchResult> {
    ...
    async call(
        {
            ...
            ttTargetIdc,
            useMobile
        }: FetchSignedWebSocketFromCustomRouteParams
    ): Promise<ProtoMessageFetchResult> {

        return this.webClient.withSigner('custom', async () => {
            if (!this.webClient.isCustomSignerEnabled) {
                throw new SignAPIError(ErrorReason.CONNECT_ERROR, undefined, undefined, 'Custom sign server is not configured.');
            }
            ...
            if (this.webClient.configuration.authenticateWs && resolvedSessionId) {
                ...
                const expectedHost = new URL(this.webClient.customSigner.configuration.basePath).host;
            ...
            try {
                response = await this.webClient.customSigner.webcast.fetchWebcastURL(
                    'ttlive-node',
                    roomId,
                    uniqueId,
