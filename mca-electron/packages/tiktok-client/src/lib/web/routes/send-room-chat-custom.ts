...
export type SendRoomChatFromCustomRouteParams = WebcastRoomChatPayload & { options?: AxiosRequestConfig };

export class SendRoomChatFromCustomRoute extends Route<SendRoomChatFromCustomRouteParams, WebcastRoomChatRouteResponse> {
    ...
    async call(
        {
            ...
            ttTargetIdc,
            options
        }: SendRoomChatFromCustomRouteParams
        ...
        return this.webClient.withSigner('custom', async () => {
            ...
            const fetchResponse = await this.webClient.customSigner.webcast.sendRoomChat({
                    roomId,
                    content,
